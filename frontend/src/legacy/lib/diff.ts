// Minimal line-based diff (LCS), no dependencies. Used by the History version view.
// "del" = line present in `a` (the base/older text) but not `b`; "add" = present in `b` (the
// selected/newer text) but not `a`; "same" = unchanged.
