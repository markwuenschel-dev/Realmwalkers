...
import { BOARD_SCENES, CHAPTERS, LANES, TIMELINE_SCENES } from "../data";
...
export default function ChaptersScreen() {
  ...
  // book summary
  const bookTarget = CHAPTERS.reduce((a, c) => a + c.target, 0);
  const bookWords = CHAPTERS.reduce((a, c) => a + c.words, 0);
  const bookPct = Math.round((bookWords / bookTarget) * 100);

  // pacing graph
  const progressChapters = CHAPTERS.map((c) => {
  ...
  const boardCards = desk.board.map((id) => {
    const s = BOARD_SCENES[id];
    const c = statusColors[s.status];
    const dragging = desk.dragId === id;
    return {
      id, no: s.no, title: s.title, words: s.words ? s.words + "w" : "—",
      statusLabel: statusText[s.status], statusColor: c,
  ...
  const lanes = LANES;
  const tScenes = TIMELINE_SCENES;
  const tCols = tScenes.length;
  ...
  tScenes.forEach((s, i) => {
    if (!seen[s.ch]) {
      ...
      const chap = CHAPTERS.find((c) => c.no === s.ch);
      chBands.push({
  ...
  const timelineNodes = tScenes.map((s, i) => {
    ...
    return {
      ...
      onClick: () => {
        if (s.status !== "planned" && s.status !== "drafting") {
          desk.openScene(s.n === 7 ? 0 : s.n === 8 ? 1 : s.n === 6 ? 2 : 0);
        }
      },
      style: `grid-column:${2 + i};grid-row:${2 + (li < 0 ? 0 : li)};align-self:center;justify-self:stretch;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;height:54px;border-radius:9px;border:1px solid ${s.n === 7 ? "var(--accent)" : "var(--line)"};border-top:3px solid ${c};background:var(--bg2);box-shadow:var(--shadow);cursor:${dim ? "default" : "pointer"};opacity:${dim ? ".5" : "1"};position:relative`,
      dot: c,
    };
  });
    ...
    <div>
      <div style={css("display:flex;align-items:flex-end;justify-content:space-between;gap:16px;flex-wrap:wrap;margin-bottom:24px")}>
        <div>
          ...
          <p style={css("margin:0;color:var(--dim);font-size:14.5px")}>Book I · The Dominion Realm — pacing against targets, and the order scenes will compile in.</p>
        </div>
      ...
      <div style={css("display:grid;grid-template-columns:300px minmax(0,1fr);gap:18px;align-items:start;margin-bottom:30px")}>
        ...
        <div style={css("background:var(--bg2);border:1px solid var(--line);border-radius:var(--r);padding:20px")}>
          ...
          <div style={css("display:flex;justify-content:space-between;font-family:var(--mono);font-size:12px;color:var(--dim);line-height:2.1")}><span>scenes approved</span><span style={css("color:var(--ink)")}>5 / 9</span></div>
          <div style={css("display:flex;justify-content:space-between;font-family:var(--mono);font-size:12px;color:var(--dim);line-height:2.1")}><span>chapters</span><span style={css("color:var(--ink)")}>3 planned</span></div>
          <div style={css("display:flex;justify-content:space-between;font-family:var(--mono);font-size:12px;color:var(--dim);line-height:2.1")}><span>awaiting you</span><span style={css("color:var(--warn)")}>2 scenes</span></div>
        </div>
        ...
        <>
          <div style={css("display:flex;align-items:center;justify-content:space-between;margin-bottom:14px")}>
            <div style={css("display:flex;align-items:center;gap:10px")}>
              <h2 style={css("margin:0;font-family:var(--display);font-weight:500;font-size:19px;color:var(--ink)")}>Chapter 2 · The Warded Door</h2>
          ...
          <div style={css("display:flex;flex-wrap:wrap;gap:12px")}>
            {boardCards.map((c) => (
              <div
                key={c.id}
                draggable
                onDragStart={() => desk.onDragStart(c.id)}
                onDragEnter={() => desk.onDragEnter(c.id)}
                onDragOver={(e) => e.preventDefault()}
                onDragEnd={desk.onDragEnd}
          ...
          <div style={css("background:var(--bg2);border:1px solid var(--line);border-radius:var(--r);padding:18px 20px;overflow-x:auto")}>
            <div style={css(`${tlGridStyle};min-width:760px`)}>
              ...
              {timelineNodes.map((nd) => (
                <div key={nd.n} onClick={nd.onClick} style={css(nd.style)}>
