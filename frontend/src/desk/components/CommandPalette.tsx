...
export default function CommandPalette() {
  ...
  const navTo = useCallback((href: string) => { closePalette(); router.push(href); }, [router, closePalette]);
  ...
  const commands: Result[] = useMemo(() => [
    ...DESK_ROUTES.map((r) => ({
      key: `go:${r.id}`, icon: r.icon, label: r.paletteLabel, hint: r.shortcut, cat: "Go", run: () => navTo(r.href),
    })),
  ...
  const results: Result[] = useMemo(() => {
    ...
    for (const c of data.chapters) {
      ...
      if (hit(`chapter ${c.chapter_no} ${c.title ?? ""} ${c.pov}`)) {
        out.push({ key: `ch:${c.id}`, icon: "▦", label: `Chapter ${c.chapter_no}${c.title ? ` — ${c.title}` : ""}`, sub: `POV · ${c.pov}`, cat: "Chapters", run: () => navTo("/chapters") });
        n++;
      }
    }
    n = 0;
    for (const e of data.canon) {
      ...
      if (hit(`${e.kind ?? ""} ${e.name ?? ""} ${e.body ?? ""}`)) {
        ...
        out.push({ key: `cn:${e.id}`, icon: "◍", label: e.name ?? "(unnamed)", sub: `${kind} · ${snippet(e.body) || "—"}`, cat: "Canon", run: () => { setLedgerCat(kind === "character" ? "characters" : `canon:${kind}`); navTo("/ledger"); } });
        n++;
      }
    }
    n = 0;
    for (const c of data.characters) {
      ...
      if (hit(`${c.character} ${JSON.stringify(c.stats)} ${c.body ?? ""}`)) {
        out.push({ key: `cs:${c.character}`, icon: "◍", label: c.character, sub: `${c.is_pov ? "POV" : "character"}${c.body ? ` · ${snippet(c.body, 50)}` : ""}`, cat: "Characters", run: () => { setLedgerCat("characters"); navTo("/ledger"); } });
        n++;
      }
    }
    return out;
  }, [q, commands, latestScenes, chapterNo, data.chapters, data.canon, data.characters, navTo, openSceneId, setLedgerCat]);
  ...
  return (
    <div onClick={togglePalette} style={css("position:fixed;inset:0;z-index:90;background:rgba(0,0,0,.62);display:flex;align-items:flex-start;justify-content:center;padding-top:13vh;animation:fadeIn .14s ease both")}>
      <div onClick={stop} role="dialog" aria-modal="true" aria-label="Command palette" style={css("width:min(600px,92vw);background:var(--bg2);border:1px solid var(--line);border-radius:13px;box-shadow:0 30px 80px rgba(0,0,0,.5);overflow:hidden;animation:fadeUp .18s ease both")}>
