...
export default function CanonCard({ card }: { card: CardModel }) {
  return (
    <span
      style={css(
        "position:absolute;left:0;top:calc(100% + 8px);width:246px;background:var(--bg2);border:1px solid var(--line);border-radius:11px;box-shadow:var(--shadow);padding:13px 14px;z-index:60;white-space:normal;text-align:left;line-height:1.45;cursor:default;font-family:var(--ui)",
      )}
    >
      <span style={css("display:block;font-family:var(--display);font-size:15px;color:var(--ink)")}>
        {card.title}
      </span>
      <span
        style={css(
          "display:block;font-family:var(--mono);font-size:9.5px;letter-spacing:.06em;text-transform:uppercase;color:var(--dim);margin:3px 0 8px",
        )}
      >
        {card.subtitle}
      </span>
      {card.rows.map((r, i) => (
        <span
          key={i}
          style={css(
            "display:flex;justify-content:space-between;gap:10px;font-size:12px;padding:4px 0;border-top:1px solid var(--hairline)",
          )}
        >
          <span style={css("font-family:var(--mono);font-size:10.5px;color:var(--dim);flex:none")}>
            {r.k}
          </span>
      ...
      {card.hasFlag && (
        ...
        >
          ...
          <span style={css("display:flex;gap:6px;margin-bottom:8px")}>
            <span
              style={css(
                "flex:1;padding:5px 8px;border-radius:6px;background:var(--bg3);border:1px solid var(--line)",
              )}
            >
              <span
                style={css(
                  "display:block;font-family:var(--mono);font-size:8.5px;text-transform:uppercase;color:var(--dim)",
                )}
              >
                prose
              </span>
          ...
          {card.open && (
            <span style={css("display:flex;gap:6px")}>
              <button onClick={card.keepProse} style={css(keepBtn)}>
                Keep prose
              </button>
              <button onClick={card.keepLedger} style={css(keepBtn)}>
                Keep ledger
              </button>
            </span>
          )}
          {card.resolved && (
            <span style={css("font-family:var(--mono);font-size:10.5px;color:var(--good)")}>
              ✓ {card.resolvedLabel}
            </span>
          )}
        </span>
