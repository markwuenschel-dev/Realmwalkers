...
> **Model:** Base class = *method* · Specialization = *refined role* · Domain = *power domain* · Legendary/Mythic/Unique titles = *rare evolved expression*.
...
> **Combat:** how each base class's *method* expresses as damage (the per-class damage-method table) → `combat_formulas.md`.
...
**Domains \= power domain** *(not to be confused with skill Affinity, the per-skill leveling mechanic in `core_rules.md`)*.  
...
| **Mage** | Direct spellcasting and supernatural manipulation | Arcanist, Elementalist, Evoker, Warder, Hexer, Illusionist, Battlemage, Spellweaver | Pyromancer, Chronomancer, Void Mage, Forceweaver, Lightweaver |
...
| **Force Knight** | Warrior \+ Force | Armored fighter who controls weight, pressure, and impact |
...
| **Eightfold Vessel** | Any base \+ Aether | Body/soul capable of containing all eight foundational domains |
...
The Realm’s class system is built around a simple principle: **classes describe how a person engages with the world, while domains describe what kind of power shapes that engagement.**
...
Domains are separate from classes. They are the supernatural sources through which power expresses itself. Fire, Water, Air, Earth, Light, Life, Shadow, and Death form the eight foundational (Elemental) domains. Runic, Psychic, Spirit, Blood, and Force represent the deeper Primordial structures. Chaos, Eldritch, Celestial, Time, Void, Entropy, and Planar represent Cosmic or reality-scale principles. Aether is not ordinary magic; it is the rare synthesis of the eight foundational domains. (Full membership: `mechanics.md` → Domain Tiers.)

Because domains are not classes, the same domain can produce very different outcomes depending on the base class and specialization. Death applied to a Warrior may produce a Death Knight. Death applied to a Mage may produce a Necromancer or Bone Mage. Death applied to a Priest may produce a Funerary Priest or Grave Saint. Death applied to a Summoner may produce a Grave Binder or Necromantic Caller. The domain is the same, but the method changes the final expression.

Likewise, similar fantasy archetypes should not automatically occupy the same system layer. A Death Knight is not a base class because it already implies a martial method, a death domain, and a developed identity. It is better understood as an evolved title: Warrior plus Knight, Champion, Guardian, or Reaver, shaped by Death. A Riftwalker (formerly "Planeswalker") is not a base class either; it is a Legendary Planar title reached through mastery of gates, boundaries, worlds, or dimensional travel. An Incarnate is not a class at all, but a mythic or unique state in which a person embodies a principle rather than merely using it.
...
Arcane refers to spellcraft: mana structure, metamagic, magical syntax, spell matrices, counterspells, wards, and casting theory. It is a discipline, not a foundational domain. Aether, by contrast, is rare synthesis: the harmonization of the foundational domains into a higher-order state. An Arcanist studies and manipulates magic’s structure. An Aetherist touches the unified substance beneath the foundational forces. The first can be a Mage specialization. The second should be exceptional, legendary, mythic, or unique.

The cleanest structure is therefore:

**Base Class** — how someone acts.  
**Specialization** — what part of that method they refine.  
**Domain** — what supernatural source empowers or flavors them.  
