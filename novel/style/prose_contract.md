# Prose Contract — Dominion Realm

> **Purpose:** Specific, binding rules for the prose. Not guidelines — contracts. If a passage violates these, it needs revision regardless of how good it feels in isolation. Cross-reference `voice_guide.md` for the philosophy behind these rules.
...
**Correct looks like:** The action, then the body's report, then what the mind does with it, even if that sequence takes half a page.

**The two modes — and which one you're in.** "What happens matters less than what it costs" is true when the *cost* is the point. It is not a license to delete the physical event when the *event* is the point. Two scenes, two modes:

- **Interiority-load-bearing** — the meaning lives in what it costs him. Compress the choreography; lead with the body's report and the mind's work. The dissociated kill (`style_examples.md` #1, #4) is this mode: *"the opening arrived and he took it and it was over"* is correct, because the scene is about his absence from his own hands, not the footwork.
- **Choreography-load-bearing** — the meaning lives in what physically happens. Show the event in full, in order, before the interpretation. The Serra duel (`style_examples.md` #11) is this mode: the tactical exchange *is* the characterization, so deleting the middle ("she planted on the wall and his block made it worse") destroys the scene. Interiority rides alongside the action here; it does not replace it.

**The test:** if a reader needs the physical steps to understand the meaning, you are choreography-load-bearing — render them in full (`prose_clarity_rules.md` R1–R4). If the steps are interchangeable and the meaning is the interior cost, you are interiority-load-bearing — compress them. Never compress choreography that carries meaning; never over-render footwork that doesn't.

---

### 2. Emotion is shown through behavior and sensation, not named.
...
**Exception:** Characters can name their own emotions in internal monologue when that naming is itself characterization — when the act of labeling is how the character copes, deflects, or lies to themselves.

---

### 3. Every physical detail earns its place or it doesn't appear.

No decorative description. Details are load-bearing or they are cut. A detail that appears must either:
- Tell us something about the character perceiving it
- Carry atmosphere that matters to the scene
...
### 4. Violence has weight and consequence. It is never free.
...
### 7. Dialogue carries subtext. Characters do not say what they mean when subtext is available.
...
### 10. Endings of scenes and chapters do not summarize. They land.
...
## Quick Reference — What the Prose Never Does
...
- Inserts humor that isn't specific to the moment
- Summarizes at scene endings

## Quick Reference — What the Prose Always Does

- Leads with interiority even in action scenes
- Lets behavior and sensation carry emotional weight
- Treats every physical detail as load-bearing or cuts it
- Marks the cost of violence, however briefly
- Keeps the UI subordinate to the scene
- Trusts subtext in dialogue
- Lands scene endings without explaining them

---

*Cross-reference: `voice_guide.md` for philosophy. `dialogue_rules.md` for spoken voice. `system_message_rules.md` for UI formatting. `style_examples.md` for applied examples. `prose_clarity_rules.md` for the show-don't-refer procedure (binding peer; arbitrated by Rule 1's two modes above).*
