---
id: roth
name: Roth
aliases: [roth]
kind: character
status: active
---
# Roth — Character File

> **Status:** Active — created ahead of need; **does NOT appear in SCENE-001** (the hijack is SCENE-002)
> **Last updated:** created from T-s01 canonization (forward-provision for SCENE-002/003)
> **File type:** Supporting (provisional)
> **Lifecycle:** provisional · **Mortality:** alive · **Canon level:** dossier (light)

---

## Identity

| Field | Value |
|---|---|
| Earth Name | Roth (canon name; kept **unnamed on-page** in the prose) |
| Realm Name | — unassigned |
| True Name | — (track carefully; metaphysically dangerous) |
| Age | TBD |
| Origin | TBD |
| Guild Role (Earth) | None — he is Astria, not a guild member |
| Affiliation | Astria — the suited figure / the hijacker who commandeers the match |
| Realm Role | — TBD / unassigned |
| Power Interface | — TBD / unassigned |
| Current Location | Earth — Astria (the launch-teaser face) |
| Current Allegiance | Astria |

---

## Why this file exists now

Roth is being canonized **ahead of his first on-page appearance** because SCENE-002 / SCENE-003 need him. He is the **suited Astria figure** who hijacks the 404 vs Dead Hand scrim — the launch-teaser face who appears in place of the Dead Hand leader when the match screens snow over and commandeers the match. **He is NOT in SCENE-001:** SCENE-001 ends *on the brink of* the hijack (the unseen rogue still hasn't shown); the intrusion itself is the start of SCENE-002. Do not retroactively place Roth on-page in SCENE-001.

> **On-page naming rule:** the canon name is **Roth**, but he is kept **unnamed in the manuscript prose** (referred to as the suited Astria figure / "Mr. Fahr"'s interlocutor). Track the name here; do not surface it on-page unless a later scene establishes it.

---

## Core Identity

Roth is institutional power wearing patience as a face. He is the man who arrives when a decision has already been made several rooms away and his job is to make it feel reasonable. His instrument is not threat but inevitability — the sense that the outcome is fixed and he is merely the polite delivery of it.

*Most of his interiority is unestablished. Do not invent psychology beyond the function the scenes give him.*

---

## Personality

### Baseline Strengths
- Composed, controlled, institutionally fluent
- Frames coercion as necessity — "Bureaucracy leaves us very little choice" (the launch-teaser / hijack line, per the SCENE-002/003 brief; **not yet canonized on-page** — confirm at SCENE-002 canonization)
- Makes the inevitable sound reasonable

### Failure Mode
*Unestablished — do not invent.*

### Hidden Weak Point
*Unestablished — do not invent.*

### What They Want to Prove
*Unestablished. His function is Astria's instrument; his personal stake is not yet on-page.*

---

## Background

### Earth Life
*TBD — established only as an Astria figure, the face of the launch teaser, with the badge/access Marcus's side of the building does not have.*

### Affiliation
Astria. He is on the side of the building "you needed a different badge to enter" — the device side, the part that produced the quantum processor marketed to "reshape the very fabric of our lives." He is the human face Astria puts on the intrusion into Marcus's private competitive life.

---

## Relationships

### Marcus (Marcus Fahr / Soren)
Antagonistic-institutional. Roth is the one who turns Astria from a distant employer into a personal violation — breaking into the scrim, addressing Marcus. The dynamic is the powerful institution reaching into the one place Marcus kept private. *Specifics develop in SCENE-002/003.*

### Seb
*TBD — Seb is the coercion target via the brother lever (SCENE-002). Roth's relationship to that lever is to be canonized when SCENE-002 is accepted.*

---

## Arc

### Book 1 Function
The face of the inciting intrusion: Astria commandeering the scrim. He carries the offer / ultimatum mechanics that move the cast from Earth toward the Realm. Lifecycle **provisional** until his first on-page scene (SCENE-002) is drafted and accepted — promote/extend then.

### Core Arc Question
*Not yet established.*

### Starting Point
*Established only as the suited Astria figure at the moment of the hijack — SCENE-002.*

### End Direction
*Unestablished — do not overcommit.*

---

## Open Questions

- Full name / title / exact role at Astria?
- Is "Roth" ever surfaced on-page, or does he stay the unnamed suited figure?
- His relationship to the device side of Astria and to whoever (or whatever) is actually steering the control-cohort outcomes Marcus caught — ⚠ likely connected; unestablished.
- Confirm the "Bureaucracy leaves us very little choice" line at SCENE-002 canonization (currently brief-sourced, not on-page canon).
- Does he recur past the Earth acts?

---

## Continuity Notes

- **Does NOT appear in SCENE-001.** First on-page appearance is SCENE-002 (the hijack). File created ahead of need.
- Canon name **Roth**; kept **unnamed in the prose**.
- Astria affiliation; the suited figure / launch-teaser face who commandeers the scrim.
- Lifecycle: provisional; mortality: alive; no Realm material assigned.
- The hijack line and offer/ultimatum mechanics are SCENE-002/003 — canonize when those scenes are accepted; do not treat as established on-page yet.
- Do not invent psychology or backstory beyond scene-established facts.

---

*Last updated: created from T-s01 canonization (forward-provision for SCENE-002/003)*

*— end of file —*
