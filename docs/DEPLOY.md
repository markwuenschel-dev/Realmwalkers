...
## Where it runs
- **Box:** EC2 `i-018796c951839031d` (t4g.medium, us-east-1), Elastic IP `44.198.76.44`. Current IP via
...
- **A fresh box starts with an empty `realmwalkers` db** — scenes/books are **not** copied up automatically.
  ...
  `realmwalkers` db.
- **Cost:** a flat EC2 bill (t4g.medium) shared across all four apps on the box, plus your own Anthropic API
