...
>
> **Second correction 2026-07-25 at HEAD `d664f87` (A1c PR) — half of Layer 1 now runs, at ONE tier.** `shared/claim_precedence.py` has a live production importer, `workers/packet/evidence.py:41`, which calls `conflict_needs_open_question` at `:190` — that is the **chapter** tier. The **scene** tier got its own objective floor in A1c: `scene_packet/blockers.automatic_hold_for_qa` raises an ApprovalBlocker with source `canon_conflict` when scene QA reports a finding in `risk_scorer.CANON_CONFLICT_KINDS`, and an active blocker refuses approval at the one approval seam. It is a **kind-based** floor, not the precedence order itself: it does not read `ClaimSource` ranks, so it escalates a *reported* conflict rather than adjudicating one the order cannot break. Wiring the scene tier to the precedence engine proper remains unowned. "Layer 1 works day one" is still false; "Layer 1 is entirely unwired" is now also false.
...
> **RETRACTED 2026-07-15 — FALSE, and it was a safety claim.** `skip_drafted: bool = True` (`workers/draft_queue.py:290`) is a **default parameter**, not a system property. A live path overrides it: `chapters.py:320` → `schedule_scene_redrafts` → `schedule_contract_first_draft_jobs(..., skip_drafted=False)` (`workers/job_scheduler.py:113-118`), which mints a **DRAFT**-kind job over a scene that already has prose, and generation then supersedes the prior scene (`pipeline.py:318`). The drafter therefore **can** run from scratch over the author's words. Any autonomous slice that relies on "revise mode only" as a safety property is relying on a default argument. This needs an explicit decision before the driver may schedule drafts.
...
> **STALE — corrected 2026-07-25 at HEAD `d664f87` (A1c PR).** Every bullet below was true at `3246a8c` and is false now: ADR-0028 Slices 2/3a′/3b and ADR-0032 W0–W2 landed in between. Verified this session — `ImportAdoption(` at `shared/adoption_entry.py:344`, `ImportSceneEvidence(` at `workers/evidence_store.py:95`, `RevisionRequest(` at `workers/revision.py:229`; `accept_revision_request` at `workers/revision.py:138`; `workers/import_adoption.py` exists; `shared/chapter_lock.py` has five importing modules; `shared/claim_precedence.py` has a live production importer at `workers/packet/evidence.py:41`; `Job.revision_request_id` is written via `workers/revision.py:264` → `workers/job_scheduler.py:110` → `workers/job_routing.py:124`. The bullets are kept verbatim below as the record of what was true when this ADR was written. **What does NOT follow:** that Layer 1 now runs at the scene tier. The live `claim_precedence` caller is `workers/packet/evidence.py` — the **chapter** tier. Scene-tier escalation arrived separately as A1c's ApprovalBlocker with an automatic `canon_conflict` source; see ADR-0033.

- `ImportAdoption(`, `ImportSceneEvidence(`, `RevisionRequest(` have **zero constructors** anywhere in `src` or `tests` — **nothing writes those three tables**. (Corrected: an earlier draft of this bullet said "nothing writes **or reads**" them. That was false — `workers/context/revision.py:26` does `session.get(RevisionRequest, job.revision_request_id)`. It is an unreachable read, because `job.revision_request_id` has no writer and is always `NULL`, but it is a reader and the bullet contradicted this ADR's own text five lines below.)
