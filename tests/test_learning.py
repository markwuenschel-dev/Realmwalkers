...
async def _scene(s, ch, scene_no=1, *, prose="Prose.", agent_original=None,
                 status=SceneStatus.PENDING_REVIEW, version=1):
    sc = Scene(chapter_id=ch.id, scene_no=scene_no, version=version, status=status,
               prose=prose, prose_source="agent", agent_original=agent_original, passes_run=["drafter"])
    s.add(sc)
    await s.flush()
    return sc
...
async def test_hand_edit_captures_rendered_agent_pair(db_factory):
    ...
    async with db_factory() as s:
        ...
        await reviews.decide(
            sc.id, DecisionIn(decision=Decision.APPROVE, edited_prose="Marcus checked his sharpened sight."),
            s, BackgroundTasks(),
        )
        await s.commit()
...
async def test_reedit_refreshes_human_text_keeps_agent_draft(db_factory):
    ...
    async with db_factory() as s:
        ...
        await reviews.decide(
            sc.id, DecisionIn(decision=Decision.APPROVE, edited_prose="First human edit."),
            s, BackgroundTasks(),
        )
        await s.commit()

        await reviews.decide(
            sc.id, DecisionIn(decision=Decision.APPROVE, edited_prose="Second human edit."),
            s, BackgroundTasks(),
        )
        await s.commit()
...
async def test_assemble_context_loads_curated_exemplars_in_order(db_factory, monkeypatch):
    ...
    async with db_factory() as s:
        ...
        s.add(PovProfile(book_id=book.id, character="Marcus",
                         exemplar_scene_ids=[str(b.id), str(a.id)]))
...
async def test_exemplars_capped_by_count_and_length(db_factory, monkeypatch):
    ...
    async with db_factory() as s:
        ...
        ids = []
        for i in range(3):
            sc = await _scene(s, ch, 10 + i, prose=f"Exemplar {i} " + "x" * 100,
...
async def test_set_exemplars_upserts_and_preserves_voice_spec(db_factory):
    async with db_factory() as s:
        ...
        await set_exemplars_mod.set_exemplars(
            s, book_title="Exemplar Book", character="Marcus", scene_ids=[sc.id]
        )
        await s.commit()
    ...
    async with db_factory() as s:
        ...
        await set_exemplars_mod.set_exemplars(
            s, book_title="Exemplar Book", character="Marcus", scene_ids=[]
        )
        await s.commit()
