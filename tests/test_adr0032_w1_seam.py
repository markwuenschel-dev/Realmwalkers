...
from dominion.shared.enums import AdoptionOperation, EntryEffect
...
async def test_unwired_operation_fails_closed(db_factory):
    """An operation not yet wired into the policy table (RECONCILIATION is W4) fails closed rather than
    guessing an intent/liveness — behaviour is defined by the command contract, never inferred."""
    async with db_factory() as s:
        _, ch = await _seed(s)
        await s.commit()
        chapter_id = ch.id
...
def _import_adoption_call_sites() -> list[str]:
    """Every `ImportAdoption(...)` CONSTRUCTOR call under src/dominion, as 'relpath:lineno'. The ORM class
    declaration (`class ImportAdoption(Base)`) is a ClassDef, not a Call, so it is not counted."""
    sites: list[str] = []
