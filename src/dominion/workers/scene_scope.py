...
def beat_keywords(beat: str) -> list[str]:
    ...
    keywords: list[str] = []
    seen: set[str] = set()
    for index, (token, is_proper) in enumerate(_beat_tokens(beat)):
        if is_proper or len(token) < 3:
            continue
        if token in _STOPWORDS or token in _DIRECTIVE_VERBS:
            continue
        if index == 0:
            # Sentence-initial token: keep only if it is clearly content (not filtered above).
            pass
...
def is_irreversible_beat(beat: str) -> bool:
    ...
    excluded so a faction name can't accidentally match a marker."""
...
def matched_beat_keywords(beat: str, prose: str) -> list[str]:
    """The beat keywords whose stems appear in the prose (whole-token, stem-aligned)."""
    stems = _prose_stems(prose)
    return [k for k in beat_keywords(beat) if any(_stems_match(_stem(k), s) for s in stems)]
...
def detect_scene_scope_bleed(
    ...
    for beat, owner in beats_owned_by_later_scenes(scene_no, sequence_body):
        matched = matched_beat_keywords(beat, prose)
        keywords = beat_keywords(beat)
        if not keywords or len(matched) < _required_hits(len(keywords)):
            continue
        irreversible = is_irreversible_beat(beat)
        issues.append(
            {
...
def detect_duplicate_irreversible_beats(
    ...
    for beat in sorted(set(owners) | forced, key=lambda b: (owners.get(b, 0), b)):
        if beat not in forced and not is_irreversible_beat(beat):
            continue
        required = _required_hits(len(beat_keywords(beat)))
        matched_by_scene = {
            scene_no: matched
            for scene_no, prose in sorted(scene_prose_by_no.items())
            if prose.strip() and len(matched := matched_beat_keywords(beat, prose)) >= required
        }
...
def evaluate_scene_scope(
    ...
    issues: list[dict[str, Any]] = []
    for scene_no, prose in sorted(scene_prose_by_no.items()):
        issues.extend(detect_scene_scope_bleed(scene_no, prose, sequence_body))
    issues.extend(detect_duplicate_irreversible_beats(scene_prose_by_no, sequence_body))
    return issues
