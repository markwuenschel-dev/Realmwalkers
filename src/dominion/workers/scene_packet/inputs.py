...
def chapter_targets(body: dict[str, Any], seeds: list[dict[str, Any]]) -> tuple[int, int | None]:
    ...
    seed_targets = [
        t for s in seeds if isinstance((wb := s.get("word_budget")), dict) and isinstance((t := wb.get("target")), int)
    ]
    chapter_target = sum(seed_targets) if seed_targets else _DEFAULT_SCENE_TARGET * len(seeds)
    return chapter_target, None


async def prior_scene_keys(session: AsyncSession, *, chapter_id: uuid.UUID, scene_no: int) -> list[list[Any]]:
