...
from dominion.shared.models import AgentOpsState, AgentPolicyOverride, LlmCall, ModelOverride
...
def _policy_from_live(agent: AgentDefinition, override: AgentPolicyOverride | None) -> AgentPolicyOut:
    ...
    never_fb = list(agent.never_fallback_tiers)
    if override and override.policy_json:
        pj = override.policy_json
        if pj.get("never_fallback") is not None:
            never_fb = list(pj["never_fallback"])
        if pj.get("fallback_tier"):
            fb_tier = pj["fallback_tier"]
...
def _agent_ops_row(agent: AgentDefinition, override: AgentPolicyOverride | None) -> AgentOpsAgentOut:
    ...
    return AgentOpsAgentOut(
        ...
        contract=AgentContractOut(
            ...
            temperature=agent.contract.temperature,
        ...
        permissions=AgentPermissionsOut(
            auto_run=agent.permissions.auto_run,
            require_approval=agent.permissions.require_approval,
            can_modify_packet=agent.permissions.can_modify_packet,
            can_block_downstream=agent.permissions.can_block_downstream,
            can_write_summaries=agent.permissions.can_write_summaries,
            can_update_canon=agent.permissions.can_update_canon,
            can_only_suggest=agent.permissions.can_only_suggest,
        ),
...
async def build_agent_stats(session: AsyncSession) -> AgentStatsListOut:
    ...
    for agent in AGENTS:
        ...
        n = len(calls)
        if n == 0:
            stats.append(AgentStatsOut(setting=agent.setting_key, label=agent.label, qa_pass_rate="—"))
            continue
